...
def Dielectric(trans_df_cut, Enmin, Enmax, kpoint_number, Volume = 200.0, sigma = 0.10, GRID = 10000, scissor = 0.0, SOC = 'FALSE'): 
    ...
    :param sigma: Width of smearing (eV)
...
def RunTransition(trans_df, ev,deltaE, option, kpt_number, k_df):
    ...
    k_df['prob'] = [0.01 for i in range(kpt_number)] #small number such that zero probability points are shown in the plot
    trans_df_plot = pd.concat([trans_df_plot,k_df], ignore_index=True) #combine both dataframes
    

 

    return trans_df_plot #return modified trans_df

def Plot(trans_df_plot): 
    ...
    fig.update_traces(marker=dict(color=trans_df_plot['prob'], size=trans_df_plot['prob']*100,colorscale='viridis', colorbar=dict(thickness=10))) #modify color/size of kpoints and add colorscale/colorbar.
...
def Interactiveplot(trans_df, tr_emin, tr_emax, tr_deltaE, deltaE, option, kpt_number, k_df ):
    ...
    for e in np.arange(tr_emin,tr_emax,tr_deltaE):
        trans_df_plot = RunTransition(trans_df, e,deltaE, option, kpt_number, k_df)
        #print(max(trans_df_plot['prob']), e)
        f=Plot(trans_df_plot)
    ...
    for i in range(len(sfig.data)):
        step = dict(
            ...
            args=[{"visible": [False] * len(sfig.data)},
                  {"title": "Slider switched to energy: " + str(2+i*.1) + "eV"}],  # layout attribute
        )
